  ...
  r.db('threereco')
    ...
    .run(connection, async (error, result) => {
      ...
      if (error) {
        response
          .status(500)
          .json({ message: 'Error while deleting user transaction.', error });
        ...
        if (result.deleted >= 1) {
          response.status(200).json({
            message: 'Deleted user transaction successfully.',
          });

          logger.success('Deleted a user transaction.');
          ...
          response
            .status(500)
            .json({ message: 'Error while deleting user transaction.' });

          logger.error('Could not delete user transaction.');
